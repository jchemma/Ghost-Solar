...
public class MainMenu extends Application{
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        int counter = 0;
        while(success){
            if(new LogIn().display()){
                break;
            }else if(counter == 3){
                new AlertBox("Too many invalid tries.", "You will be locked out for 1 minute.").display();
                Thread.sleep(10000);
            }
            else{
                new AlertBox("The password is incorrect", "I'm sorry").display();
                counter++;
            }
        }
